...
This is the create step, separate from the pipeline. After the project exists, the user runs /omero-design-partner and /omero-project-setup in either order (independent prerequisites: design writes the feature sheet(s), setup proves the project environment ready), then /omero-build-loop builds the sheet. Do NOT run those here; this skill only creates the project.
