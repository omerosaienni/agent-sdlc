...
The installer is deterministic: it copies the path-scoped stack rules
(omero-typescript.md, omero-mongo.md, omero-react.md) from the agent-sdlc
templates into the target repo's .claude/rules/, where Claude Code reads them
automatically. Pass the repo path and one or more stack flags for the stacks that
...
Report that to the user. On a non-zero exit, report the error (no stack flag given,
or the target is not a git repository) so the user can correct and re-run.
