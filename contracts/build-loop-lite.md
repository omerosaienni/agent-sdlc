...
1. Integration: run the full accumulated integration suite once against the finished main; it must pass. The endpoint is needed once, here, not while iterating. A failure is a real behaviour problem, but the offending increment is already `merged` (the gate only runs once every increment is merged) and `merged` is terminal, so the loop never silently mutates it: the failure ESCALATES the feature (build-judge-loop.md, Escalation), and the human resolves it by appending a fix increment to the sheet, which the loop then builds, verifies and integrates through the normal per-increment cycle like any other increment. The gate re-runs once that fix is merged. Completion waits until the suite is green.
