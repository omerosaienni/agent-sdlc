...
7. **Required remote checks pass.** `gh pr checks <n> --required` must exit 0. A non-zero exit means a required check is pending or failing, which BLOCKS. GOTCHA: with zero required checks it exits 0 with empty output; that is the no-checks-configured case, distinct from a red check. The script WARNs and continues on empty output rather than blocking, so a repo that has no required checks can still ship; the warning tells the operator nothing was verified remotely, and they can rely on the optional local CI step (gate 8) or a manual check. Pending or failing checks always BLOCK.
