...
- The type-check runner (.building/scripts/agent-typecheck.sh) is present, matches the shared template, and runs clean. Setup places it under .building/scripts/ if absent or out of date, then runs it: a clean type-check (exit 0) is REQUIRED, type errors are a hard FAIL (NOT READY, the same standing as a failing test), because the judge gates on it and the tiers do not type-check (they strip types). A type-check that cannot run at all (no tsconfig, tsc absent, a config error: exit 3) is also NOT READY, a static-environment gap to fix before building.
