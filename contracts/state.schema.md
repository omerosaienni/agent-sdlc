...
9. completion, if present, occurs only when profile is explicitly lite; its integration is one of `pending`/`passed`/`failed` and its docs is one of `pending`/`pr-open`/`merged`. (It is created once every increment is first merged, but it then legitimately persists while a fix increment appended after an integration failure is in flight, so it may coexist with non-merged increments; this is not a validation constraint.)
