...
On entry:
- Check for the setup receipt .building/setup-ok. Absent: STOP and tell the user to run /omero-setup-project first (the loop never runs setup itself).
...
## Branch and PR
- One branch per increment, named to the project branch-naming standard (claude-rules/omero-branch-naming.md): `feat/<id>-<kebab-title>` (e.g. feat/claude-metrics_03-metrics-persist-jsonl-sink, the `feat` type then the full increment id claude-metrics_03-metrics-persist plus its kebab title), so the branch list is an audit trail. Increments deliver a feature, so the type is always `feat`. The work folder and records are keyed by this branch name. No worktree, in either mode.
