...
## Location
`.building/work/<branch-name>/doc-payload.md`, gitignored working file, written as the agents work.
...
- key decisions: each a decision plus why, the non-obvious ones only. [builder, plus the reviewer report's architecture section]
...
The agent does NOT re-read all the code; it reads these plus a quick scan of the public interface to confirm.

## Degradation
If the builder slice is absent or partial, the document agent fills what it can from the reports and marks the missing fields "not provided". It never blocks. The reviewer separately notes a missing slice as a non-blocking suggestion, so quality is visible without compromising the never-block guarantee.
