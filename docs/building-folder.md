...
**epics/** holds the epic manifests the design partner writes, one folder per epic keyed by a short kebab-case epic name, each with an `epic.json` (its structure is [`../contracts/epic-manifest.schema.md`](../contracts/epic-manifest.schema.md)). A manifest lists a run's features in build order with their cross feature dependencies: it is where you read off what to build first when a run spans several features. It sits above `features/` because it spans them, and it references each feature by its existing `features/<feature-name>/` path rather than owning the folder. This is a human build order reference you consult to drive the build yourself; the build loop never reads it and nothing in the loop is gated on it, so it stays purely on your side of the pipeline. Emitted every design run, even a single feature one.
...
**work/** is where the agents record what they did, one folder per unit, named after that unit's audit-named branch (so `feat/claude-metrics_03-metrics-persist-jsonl-sink`, not an opaque `build/3`). The builder, the reviewer and the judge each write their file here (the builder also writes the documentation slice, `doc-payload.md`, for the document agent); the document agent reads these but writes its output into the committed `docs/` tree, not here. These files ARE the record: there is no second copy kept anywhere. Re-running a unit overwrites its folder, only the latest run is kept. Within one run, the review passes accumulate (review-pass-1, review-pass-2, and so on) so you can see the back-and-forth.
