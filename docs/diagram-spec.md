...
### file-layout.svg
Two boxes: committed versus gitignored. Committed (green): the deliverable, code plus docs/ (modules and ARCHITECTURE.md). Gitignored (grey): the whole .building/ folder, all loop output, never committed. Inside .building/: project-level setup-ok and scripts/ (the three runners) at the top, then build/ with one folder per design (deliverable queue); each build/<design-name>/ holds its own state.json (carrying that queue's mode), a work/<branch-name>/ tree per unit (builder.md, review-pass-N.md, judge.md, doc-payload.md; an escalations/ subfolder if escalated), and that queue's escalations/ index of relative symlinks into its own work folders. One gitignore rule: .building/. Shows the file-based, no-memory architecture and the per-design state layout.
...
### git-topology.svg
Branch topology of sequential-attended mode. One horizontal main line (grey). Each deliverable cuts a branch (blue) from a freshly-fetched origin/main, named after the work (<id>-<kebab-title>, e.g. d3-connection-helper, for audit), does the work, and opens a PR back into main (dark arrow) which the human merges. The next deliverable cuts fresh from the updated origin/main only after the previous PR merged. There is no integration branch and no second hop. main is green after every merge; each PR is one deliverable's audit record. Caption explains the per-deliverable cut, build, review, judge, document, PR, merge, and that this is the sequential mode; the parallel counterpart is parallel-topology.svg.
