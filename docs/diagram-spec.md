...
Canonical definition of the SDLC visual set. The SVGs in docs/ are generated artifacts; this spec is their source of truth. To regenerate or update a diagram, bring this spec into a Claude web conversation (where SVG renders inline) and say "regenerate the data-flow" or "update the loop diagram to add X" or "add a new diagram of Y". When a diagram changes, update the affected SVG AND its catalogue entry here, so the spec never drifts from the diagrams.
