...
### git-topology.svg
Branch topology. One horizontal main line (grey). Each deliverable cuts a branch (blue) from main, named after the work (<id>-<kebab-title>, e.g. d03-connection-helper, for audit), does the work, and opens a PR back into main (dark arrow) which the human merges. The next deliverable cuts fresh from the updated main. There is no integration branch and no second hop. main is always green; each PR is one deliverable's audit record. Caption explains the per-deliverable cut, build, review, judge, document, PR, merge.
