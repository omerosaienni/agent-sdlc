...
1. **Design** (`/omero-design-sheet`) converges a fuzzy intent into a feature sheet per feature, written to `.building/features/<feature-name>/increments.md` where `<feature-name>` is a short kebab-case name you give the feature, plus one epic manifest at `.building/epics/<epic-name>/epic.json` ([`../contracts/epic-manifest.schema.md`](../contracts/epic-manifest.schema.md)) listing the run's features in build order. The manifest is your build order reference across features; the build loop never reads it. Gate: the sheet, validated against [`../contracts/increment-sheet.schema.md`](../contracts/increment-sheet.schema.md). The mechanical gate cannot see design-level soundness across increments (contradictions, dead-code cuts), so `/omero-review-sheet` ([`../contracts/design-review.md`](../contracts/design-review.md)) reviews the emitted sheet for those before build; run it after design and re-converge on a bounce.
...
3. **Build** (`/omero-build-full`) delivers the sheet one increment at a time. It checks the receipt on entry, then for each increment runs builder, reviewer, tiered judge, document, then a PR and your merge with a remote, or a local commit integrated into main without one. The loop runs in one of two modes, sequential-attended or parallel-attended, read from `mode` in `state.json`, and one of two profiles, full (the default) or lite, read from `profile`: lite defers the integration tier and the documentation to a completion gate. A fast variant, `/omero-build-quick`, verifies with the type-check and unit tier only. Two unattended variants, `/omero-build-full-stacked` and `/omero-build-quick-stacked`, remove the human from the build queue, stacking increments as one linear git-town stack and proposing PRs they never merge (a remote is required). See [`build-loops.md`](build-loops.md).
...
- `scripts/validate-epics.sh` validates the design phase's epic manifest against the manifest schema; it gates a build order reference the loop never reads, not a loop input.
...
The [unattended stacked path](build-loops.md#the-unattended-path-stacked-builds) has two stricter prerequisites the attended run does not: a GitHub remote is **mandatory** (there is no local-only stacked flow, so a missing remote stops the loop rather than warning), and **git-town must be installed** (it is the tool the stack is built with). Everything else is the same setup gate and receipt.
