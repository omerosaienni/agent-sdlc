...
The thin `/omero-*` wrappers, in [`../skills/`](../skills/). `omero-create-ts-project` runs the TypeScript project generator (TypeScript base, optional `--mongo`, `--react` and `--express` layers) and `omero-create-python-project` runs the Python generator (src-layout, uv, pytest, strict pyright); `omero-install-project-rules` installs stack rules into a repo and `omero-install-global-rules` runs the once-per-machine global setup (symlinks the global rules and installs the git guards); the pipeline skills (`omero-design-partner`, `omero-project-setup`, `omero-build-full`) each reference a contract by path. All carry `disable-model-invocation: true`; invoke with `/omero-*`.
